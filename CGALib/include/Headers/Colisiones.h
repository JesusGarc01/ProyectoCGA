...
void addOrUpdateColliders(		//MOD 	|| RECIBE TIPO DE HONGO
		std::map<std::string, std::tuple<AbstractModel::SBB, glm::mat4, glm::mat4, char> > &colliders,
		std::string name, AbstractModel::SBB collider, glm::mat4 transform, char tipo) {
	std::map<std::string, std::tuple<AbstractModel::SBB, glm::mat4, glm::mat4, char> >::iterator it =
			colliders.find(name);
	if (it != colliders.end()){
		std::get<0>(it->second) = collider;
		std::get<2>(it->second) = transform;
		std::get<3>(it->second) = tipo;
	}else
		colliders[name] = std::make_tuple(collider, glm::mat4(1.0), transform, tipo);
}

void addOrUpdateColliders(std::map<std::string, std::tuple<AbstractModel::SBB, glm::mat4, glm::mat4> > &colliders,
								std::string name) {
...
bool testOBBSBB(AbstractModel::OBB a, AbstractModel::OBB b){
	float EPSILON = 0.0001;
	float ra, rb;
	glm::mat3 R = glm::mat4(0.0), AbsR = glm::mat4(0.0);
	// compute the matrix axis from the quaternions
