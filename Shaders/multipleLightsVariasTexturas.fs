...
// nueva variable colocada para el ejersicio
...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	vec4 ColorMezclado= mix(colorText1, colorText2, 0.5);
	// Ambient
    vec3 ambient  = light.ambient * vec3(ColorMezclado);
  	
    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse  = light.diffuse * (diff * vec3(ColorMezclado));
    ...
    vec3 specular = light.specular * (spec * vec3(ColorMezclado));  
